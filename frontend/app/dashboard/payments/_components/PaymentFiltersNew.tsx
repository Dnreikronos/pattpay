...
type FilterCategory = 'main' | 'status' | 'date'
...
export function PaymentFilters({ filters, onFiltersChange }: PaymentFiltersProps) {
  ...
  const hasActiveFilters = (filters.statuses && filters.statuses.length > 0) || !!filters.search || !!filters.datePreset
  ...
  const statusIcons: Record<PaymentStatus, React.ReactNode> = {
    ...
    pending: <CheckCircle2 className="h-3 w-3" />,
  ...
  const renderFilterChips = () => {
    const hasChips = filters.search || (filters.statuses && filters.statuses.length > 0) || filters.datePreset
        ...
        <AnimatePresence mode="popLayout">
          ...
          {hasActiveFilters && (
            <motion.button
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0, opacity: 0 }}
              onClick={() => onFiltersChange({ statuses: [], search: '', datePreset: undefined })}
        ...
        <AnimatePresence mode="wait">
          ...
          {currentView === 'status' && (
            <motion.div
              ...
              className="space-y-1 p-3"
            >
              {(['success', 'pending', 'failed'] as PaymentStatus[]).map((status) => {
